...
class AppStore extends Component {
  ...
  getSearchedApps = filteredApps => {
    const {searchedInput} = this.state
    const searchedResults = filteredApps.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchedInput.toLowerCase()),
    )
  ...
  render() {
    const {activeTabId} = this.state
    const filteredApps = this.getFilteredAppsAccToActiveTab()
    const searchedAppsInEachTab = this.getSearchedApps(filteredApps)
    return (
        ...
        <ul className="tab-items-container">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              onClickTabItemChangeActiveTab={this.onClickTabItemChangeActiveTab}
              isActive={activeTabId === eachTabItem.tabId}
            />
          ))}
        </ul>
        <ul className="apps-container">
          {searchedAppsInEachTab.map(eachAppItem => (
